<?php
    ...
    include_once $_SERVER['DOCUMENT_ROOT'].'/lo/common/session.php';
	...
	class myMember{
		...
		protected function dbConnection(){
			include_once $_SERVER['DOCUMENT_ROOT'].'/lo/connect/connect.php';
		}
		...
		function signUp(){
            ...
            $profilePhoto = '';
            if($gender == 'm'){
                $profilePhoto = '/lo/images/me/boy.png';
            }else if($gender == 'w'){
                $profilePhoto = '/lo/images/me/girl.png';
            }

            //기본 커버 사진 설정
            $coverPhoto = '/lo/images/me/happyCat.png';

            //회원가입 시간
            $regTime = time();

            //데이터베이스에 입력
            $sql = "INSERT INTO websMember(userName, email, pw, birthDay, gender, profilePhoto, coverPhoto, regTime) "; //마지막 공백 입력.
        ...
        function emailCheck($email){
            ...
            if(filter_var($email, FILTER_VALIDATE_EMAIL)){
                ...
                $sql = "SELECT * FROM websMember WHERE email = '{$email}'";
                $this->dbConnection();
                $res = $this->dbConnection->query($sql);
